...
8. [プリセット](#プリセット)
9. [高度な機能](#高度な機能)
10. [パラメータ詳細説明](#パラメータ詳細説明)
11. [使用例とテクニック](#使用例とテクニック)
...
Quad Morph Filter のメインウィンドウは、常時表示される上半分と、3つのタブで切り替わる下半分に分かれています。

```
┌──────────────────────────────────────────────────────────┐
│ ヘッダー                                                  │
│   タイトル      ◀ [プリセット名] ▶  PRESET RANDOM RESET  │
├───────────────────────────────┬──────────────────────────┤
│ フィルター応答カーブ           │ XY モーフパッド           │
│  ・A/B/C/D の個別カーブ        │  ・4隅が A/B/C/D          │
│  ・合成カーブ（主線）          │  ・LFO1/2/3 タブで切替    │
│  ・カットオフ位置マーカー      │  ・ドラッグで軌跡を記録   │
├───────────────────────────────┴──────────────────────────┤
│ [ FILTER ] [ MOD ] [ CONFIG ]                            │
├──────────────────────────────────────────────────────────┤
│ FILTER タブ                                               │
...
│     CUTOFF / RESONANCE / LFO2 / LFO3        × 4 行        │
│   OUTPUT                                                 │
│     Output Gain / Dry-Wet / Ceiling（ノブ）              │
│     INFO 欄（マウスを乗せたコントロールの説明）          │
├──────────────────────────────────────────────────────────┤
│ MOD タブ                                                  │
...
| **ドラッグ** | XYパッド（Recording 選択時） | 軌跡を記録 |
| **スライダードラッグ** | パラメータ | 値をスムーズに変更 |
| **ドラッグ（上下／左右）** | ノブ | 値を変更 |
| **Ctrl + クリック** | スライダー / ノブ | デフォルト値にリセット |
| **クリック** | S ボタン | そのフィルターだけを鳴らす（Solo） |
| **クリック** | LFO2 / LFO3 ボタン | Off → +X → +Y → -X → -Y を順に切替 |
| **マウスオーバー** | 任意のコントロール | FILTER タブの INFO 欄に説明を表示 |
| **クリック** | プリセット一覧の ★ | お気に入りに追加／解除 |
| **右クリック** | ユーザープリセット | 削除メニュー |

> **V1.1.0 での変更:** 背景色をランダム化する *E-ボタン* は廃止しました。
> 配色は CONFIG タブの 10 種類のカラーテーマから選びます。
...
ヘッダー右側の **PRESET** ボタンでブラウザが開きます。

### ブラウザの構成

左から「カテゴリ」「サブカテゴリ」「一覧」の3カラムです。

| カテゴリ | 内容 |
|---|---|
| **All** | ファクトリーとユーザーの両方 |
| **Factory** | 内蔵の200プリセットのみ |
| **User** | 自分で保存したもののみ |
| **Favorites** | ★を付けたものだけ |

一覧の**名前をクリックすると即座に読み込まれます**（ダブルクリックは不要です）。左端の**★をクリックするとお気に入りの追加／解除**ができます。検索ボックスに文字を入れると名前で絞り込めます。

お気に入りは `Documents/OTODESK/QuadMorphFilter/Presets/_favorites.txt` に保存され、複数のプラグインインスタンスで共有されます。書き込みのたびにファイルを読み直してマージするので、2つのウィンドウを同時に開いていても片方の追加が消えることはありません。
...
| カテゴリ | 狙い |
|---|---|
| Classic Analog | Moog / CEM3320 / SSM2040 / Jupiter / SEM / CS-80 の定番 |
| Acid & Squelch | TB-303 中心。高レゾナンス＋同期LFO |
| Vocal & Formant | Vowel Filter で母音間をモーフ |
| Rhythmic & Gated | 同期LFOをゲートとして使う |
| Ambient & Space | FDN Reverb / Waveguide / Modal で空間を作る |
| Metallic & Resonant | Comb / Modal / Waveguide の共鳴 |
| Lo-Fi & Digital | Bitcrush / Nyquist AA のざらついた質感 |
| Motion & Morph | Spread や2D波形で4フィルターを回す |
| FX & Special | Phaser / Bode Shifter / Wavefolder / Phased Array |
| Bass & Sub | 低域主体。レゾナンス控えめ |

ヘッダーの **◀ ▶** で1つずつ順送りできます。端まで行くと折り返します。現在のプリセット名は2つのボタンの間に表示されます。

### 自分のプリセットを保存する

ブラウザ下部に名前を入力し、必要ならサブカテゴリも入れて **Save** を押します。保存先は `Documents/OTODESK/QuadMorphFilter/Presets` で、サブカテゴリはそのままフォルダ名になります。ユーザープリセットは**右クリックで削除**できます。

### ブラウザの Random ボタン

**いま一覧に表示されているものから1つをランダムに選んで読み込みます。** カテゴリ・サブカテゴリ・検索の絞り込みがそのまま効くので、「Ambient & Space の中だけでシャッフル」「お気に入りの中から1つ選ぶ」といった使い方ができます。

ヘッダーの **RANDOM** とは別機能なので注意してください。

### ヘッダーの RANDOM ボタン（新規生成）

全パラメータに一様乱数をばら撒くと、ほぼ確実に使えない音になります。そこで4つの制約を掛けています。

まず**キャラクターを1つ選びます**。28モデルから独立に4本引くのではなく、Analog / Aggressive / Resonant / Clean / Digital / Spatial という相性のよい6グループから1つ選び、その中だけでモデルを振ります。次に**4本のカットオフを対数軸で低→高に分担**させ、同じ帯域で潰し合わないようにします。**レゾナンスは有効フィルター本数が多いほど上限を下げ**、高域側ほどさらに抑えます。LFOは**テンポ同期を基本**とし、周期も8小節〜1拍の実用域に限定します。

Output Gain / Dry-Wet / Ceiling は音量事故を避けるため**ランダム化されません**。

### RESET ボタン

4フィルター・5つのLFO・エンベロープフォロワー・モーフ設定を含む**すべてのパラメータを初期値に戻します**。確認ダイアログが出るので、誤って押しても Yes を選ばなければ何も起きません。カラーテーマだけは音に関係しないため対象外です。

### 保存される情報

プリセット名・ファクトリー番号・ウィンドウサイズはプラグインの状態に含まれます。DAWのプロジェクトを保存して開き直しても、プラグインウィンドウを閉じて開き直しても、そのまま復元されます。

Solo は一時的なモニタリング機能なので**保存されません**。プロジェクトを開き直すと必ず解除された状態になります。

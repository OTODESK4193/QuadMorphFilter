...
#include "../PluginProcessor.h"   // 前方宣言の実体はここで解決
...
void FilterVisualizer::paint(juce::Graphics& g)
{
    ...
    // dB 表示範囲
    // +60dB: Res=10 の共振ピークが収まる
    // -120dB: 96dB/oct スロープが収まる
    ...
    for (int px = 0; px <= wInt; ++px)
    {
        ...
        auto calc = [&](juce::String s, int idx) -> float
            {
                ...
                {
                    int stages = (slopeIdx == 0) ? 1 : (slopeIdx == 1) ? 2
                        : (slopeIdx == 2) ? 4 : 8;
                    ...
                    float m = 1.0f / den;
                    if (t == 1) m *= w_norm; else if (t == 2) m *= w2;
                    else if (t == 3) m *= std::abs(1.0f - w2);
                ...
                {
                    int stages = (slopeIdx == 0) ? 1 : (slopeIdx == 1) ? 1
                        : (slopeIdx == 2) ? 2 : 4;
                    ...
                    if (slopeIdx == 0) {
                        if (t == 1) mag *= w_norm; else if (t == 2) mag *= w2;
                        else if (t == 3) mag *= std::abs(1.0f - w2);
                    }
                    else {
                        if (t == 1) mag *= w2; else if (t == 2) mag *= w2 * w2;
                ...
                else if (modelIdx == 2)
                {
                    int stages = (slopeIdx == 0) ? 1 : (slopeIdx == 1) ? 1
                        : (slopeIdx == 2) ? 2 : 4;
                    float r_diode = juce::jmap(juce::jlimit(0.1f, 10.0f, res),
                        0.1f, 10.0f, 0.0f, 15.0f);
                    ...
                    if (slopeIdx == 0) {
                        ...
                        if (t == 1) mag *= w2 * w_norm; else if (t == 2) mag *= w2 * w2;
                        else if (t == 3) mag *= std::abs(1.0f - w2 * w_norm);
                    }
                    mag *= (1.0f + 0.2f * r_diode);
                }
                else if (modelIdx == 5)
                {
                    float v = juce::jmap(std::log10(freqLimit),
                        std::log10(20.0f), std::log10(20000.0f), 0.0f, 4.0f);
                    ...
                    for (int f = 0; f < 3; ++f) {
                        ...
                        float m_f = 1.0f / std::sqrt(std::pow(1.0f - w_f * w_f, 2.0f)
                            + std::pow(w_f * d_f, 2.0f));
                        if (t == 1) m_f *= w_f; else if (t == 2) m_f *= w_f * w_f;
                        else if (t == 3) m_f *= std::abs(1.0f - w_f * w_f);
                        mag_sum += m_f * gains[f];
                    }
                ...
                else if (modelIdx == 7)
                {
                    int stages = (slopeIdx == 0) ? 1 : (slopeIdx == 1) ? 2
                        : (slopeIdx == 2) ? 4 : 8;
                    float ms_res = juce::jmap(juce::jlimit(0.1f, 10.0f, res),
                        0.1f, 10.0f, 0.0f, 2.5f);
                    float d = 1.0f / (ms_res + 0.1f);
                    float den = std::sqrt(std::pow(1.0f - w2, 2.0f)
                        + std::pow(w_norm * d, 2.0f));
                    float m = 1.0f / den;
                    if (t == 1) m *= w_norm; else if (t == 2) m *= w2;
                    else if (t == 3) m *= std::abs(1.0f - w2);
                ...
                {
                    int stages = (slopeIdx == 0) ? 2 : (slopeIdx == 1) ? 4
                        : (slopeIdx == 2) ? 8 : 16;
                    float phi = -2.0f * std::atan(freq / juce::jlimit(20.0f, 20000.0f, fc));
                    float fb = juce::jmap(juce::jlimit(0.1f, 10.0f, res),
                        0.1f, 10.0f, 0.0f, 0.95f);
                    if (t == 1 || t == 3) fb = -fb;
                    float c_ap = std::cos(stages * phi);
                    float s_ap = std::sin(stages * phi);
                    float den2 = (1.0f - fb * c_ap) * (1.0f - fb * c_ap)
                        + (fb * s_ap) * (fb * s_ap);
                ...
                else if (modelIdx >= 17 && modelIdx <= 20)
                {
                    ...
                    for (int k = 0; k < sections; ++k)
                    {
                        ...
                        float stage_w = freq / juce::jlimit(20.0f, 20000.0f,
                            freqLimit * freqScale);
                        float stage_w2 = stage_w * stage_w;
                        float stage_d = 1.0f / stage_q;
                        float den = std::sqrt(std::pow(1.0f - stage_w2, 2.0f)
                            + std::pow(stage_w * stage_d, 2.0f));
                        float m = 1.0f / den;
                        if (modelIdx == 20) { m = std::abs(1.0f - stage_w2 * 0.5f) / den; }
                        else {
                            if (t == 1) m *= stage_w; else if (t == 2) m *= stage_w2;
                            else if (t == 3) m *= std::abs(1.0f - stage_w2);
                        }
                        mag_total *= m;
                    }
                    mag = mag_total;
                ...
                {
                    float x_zp = juce::jlimit(0.0f, 1.0f, juce::jmap(std::log10(fc),
                        std::log10(20.0f), std::log10(20000.0f), 0.0f, 1.0f));
                    float y_zp = juce::jlimit(0.0f, 1.0f,
                        juce::jmap(res, 0.1f, 10.0f, 0.0f, 1.0f));
                    ...
                    float mag_total = 1.0f;
                    for (int k = 0; k < 7; ++k) {
                        float stage_fc = fA[k] * (1 - x_zp) * (1 - y_zp)
                            + fB[k] * x_zp * (1 - y_zp)
                            + fC[k] * (1 - x_zp) * y_zp
                            + fD[k] * x_zp * y_zp;
                        float stage_q = std::max(0.5f,
                            qA[k] * (1 - x_zp) * (1 - y_zp)
                            + qB[k] * x_zp * (1 - y_zp)
                            + qC[k] * (1 - x_zp) * y_zp
                            + qD[k] * x_zp * y_zp);
        ...
        // 等パワーwMix でビジュアライザーも ProcessorBlock と一致させる
    ...
    for (int px = 0; px <= wInt; ++px)
    {
        ...
        yPos = juce::jlimit(0.0f, h, yPos);  // 描画範囲外への逸脱を防ぐ
